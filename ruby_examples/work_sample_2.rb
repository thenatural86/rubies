sig { params(units: T.nilable(String), _column_name: Symbol, currency: T.nilable(String), time: T.nilable(String)).returns(String) }
  ...
  def display_value(value, currency, _column_name, time)
   ...
   if T.must(value).negative?

    time_value_to_display = T.must(Localization.format_currency(value, currency_code: currency)).delete_prefix('$').delete_prefix('€').delete_prefix('£')
    time_value_to_display[1] = ''
    value_to_display = time ? time_value_to_display : Localization.format_currency(value, currency_code: currency)
    return T.must(value_to_display)
   else
    time_value_to_display = T.must(Localization.format_currency(value, currency_code: currency)).delete_prefix('$').delete_prefix('€').delete_prefix('£')
  ...
  sig { params(value: T.nilable(String), units: T.nilable(String), currency: T.nilable(String), column_name: Symbol, time: T.nilable(String)).returns(T.any(String, NilClass)) }
  def display_value_and_currency(value, units, currency, column_name, time = nil)
    ...
    if value[0] == '-'
      ...
      unit_display = display_units(units, column_name, currency, time)
      return "#{value_to_display} #{unit_display}"
    else
      value_to_display = value.delete_prefix('$').delete_prefix('€').delete_prefix('£')
      unit_display = display_units(units, column_name, currency, time)
      return "#{value_to_display} #{unit_display}"
    end
  end
